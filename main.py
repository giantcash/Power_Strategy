...
# 標註進出場
ax1.scatter(np.where(df['Entry'])[0], df.loc[df['Entry'], 'min']-100, marker='^', color='orange', s=150, label='Up')
ax1.scatter(np.where(df['Exit'])[0], df.loc[df['Exit'], 'max']+100, marker='v', color='darkblue', s=150, label='Down')

ax1.set_title(f"TX Continuous - Power Strategy Daily Analysis")
