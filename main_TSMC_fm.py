...
# --- 資料抓取：使用 Findmind API ---
print("正在從 Findmind 抓取台積電期歷史資料...")
now_date = now.date()
start_date_fm = (now_date - timedelta(days=110)).strftime('%Y-%m-%d')

# 每天日盤收盤才執行，end_date 直接用今天
...
df_fm_raw = fm_loader.taiwan_futures_daily(
    futures_id="CDF",      # 台積電個股期
    start_date=start_date_fm,
    end_date=end_date_fm
)

if df_fm_raw.empty:
    print("❌ Findmind 回傳空資料，程式結束。")
    sys.exit()

# --- Findmind 資料過濾邏輯 ---
# (1) trading_session = position (日盤正規盤)
df_fm = df_fm_raw[df_fm_raw['trading_session'] == 'position'].copy()

# (2) contract_date: 每個月第三個禮拜三(包含)使用當月，之後使用次月
...
# (3) 收盤價(結算價): if settlement_price != 0 請使用 settlement_price, else close
...
# 去重並排序
df = df_fm.drop_duplicates(subset=['date'], keep='first').sort_values('date').reset_index(drop=True)
...
# --- 權勢策略指標計算 ---
# 每天日盤收盤後執行，所有 K 棒皆為歷史完整資料，直接套用正常計算邏輯
df['RL'] = np.nan
df['BL'] = np.nan
df['Signal'] = 0
df['Message'] = "⚖️【中性觀望】"
...
if public_img_url:
    ...
    # Telegram 訊息 (收盤後執行，固定標題)
    message = (f"📊 *台積電期權勢策略(收盤)*\n"
